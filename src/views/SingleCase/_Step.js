        ...
        return <li className="test">
            <Card>
                <CardBlock className={`card-body`}>
                    <div className="actionsMenuToggler" onClick={e => this.setState({ menuActionOpen: true })}>&#9776;</div>
                        ...
                        <Row>
                            <Col xs="3">
                                <FormGroup>
                                    ...
                                    <Input
                                        onChange={(e) => this.props.handleChange(e)} name={`steps.${order - 1}.type`} type="select" id={`action-${order}`} defaultValue={type}
                                        required
                                    >
                    ...
                    {this.state.menuActionOpen && <div className='actions' >
                        <div className="actionsMenuToggler inheritColor" onClick={e => this.setState({ menuActionOpen: false })}>X</div>
                        <i className="fa fa-clone" aria-hidden="true" onClick={() => this.props.duplicateStep(step)}></i>
                        <i className="fa fa-arrow-up" aria-hidden="true" onClick={() => this.props.move(step.order, -1)}></i>
                        <i className="fa fa-arrow-down" aria-hidden="true" onClick={() => this.props.move(step.order, 1)}></i>
                        <i className="fa fa-trash" aria-hidden="true" onClick={() => this.props.deleteStep(step.order)}></i>
                    </div>}
                </CardBlock>
            </Card>
        </li>
